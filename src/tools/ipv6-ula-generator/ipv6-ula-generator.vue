...
const calculatedSections = computed(() => {
  ...
  return [
    ...
    {
      label: t('toolContent.ipv6Ula.firstBlock'),
      value: `${ula}:0::/64`,
    },
    {
      label: t('toolContent.ipv6Ula.lastBlock'),
      value: `${ula}:ffff::/64`,
    },
  ];
});

const addressValidation = macAddressValidation(macAddress, () => t('toolContent.mac.invalid'));
</script>

<template>
  <div>
    <n-alert :title="$t('toolContent.ipv6Ula.info')" type="info">
      {{ $t('toolContent.ipv6Ula.description') }}
    </n-alert>
