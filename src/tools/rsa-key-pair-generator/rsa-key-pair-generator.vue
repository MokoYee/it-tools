...
const { attrs: bitsValidationAttrs } = useValidation<number>({
  ...
  rules: computed(() => [
    {
      message: t('toolContent.rsa.invalidBits'),
...
<template>
  <div style="flex: 0 0 100%">
    <div item-style="flex: 1 1 0" style="max-width: 600px" mx-auto flex gap-3>
      <n-form-item :label="$t('toolContent.rsa.bits')" v-bind="bitsValidationAttrs as any" label-placement="left" label-width="100">
        <n-input-number v-model:value="bits" min="256" max="16384" step="8" />
      </n-form-item>

      <c-button @click="refreshCerts">
        {{ $t('toolContent.rsa.refresh') }}
      </c-button>
    </div>
  </div>

  <div>
    <h3>{{ $t('toolContent.rsa.publicKey') }}</h3>
    <TextareaCopyable :value="certs.publicKeyPem" />
  </div>

  <div>
    <h3>{{ $t('toolContent.rsa.privateKey') }}</h3>
