  ...
  <div>
      ...
      <div v-else>
        ...
        <div flex items-center justify-between gap-2>
          <c-button :disabled="!isMediaStreamAvailable" @click="takeScreenshot">
            ...
            {{ $t('toolContent.camera.takeScreenshot') }}
          </c-button>
    ...
    <div grid grid-cols-2 mt-5 gap-2>
      <c-card v-for="({ type, value, createdAt }, index) in medias" :key="index">
        <img v-if="type === 'image'" :src="value" max-h-full w-full :alt="$t('toolContent.camera.screenshot')">

        <video v-else :src="value" controls max-h-full w-full />

        <div flex items-center justify-between>
          <div font-bold>
            {{ type === 'image' ? t('toolContent.camera.screenshot') : t('toolContent.camera.videoName') }}
          </div>
